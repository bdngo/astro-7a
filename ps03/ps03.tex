...
\begin{equation}
    F_\lambda = \frac{L}{A} = \frac{E}{A t \lambda}
\end{equation}
...
\begin{center}
    \begin{tikzpicture}
        \begin{axis}[
            ...
            xlabel={\(\lambda\) [\si{\centi\meter}]},
            ylabel={\(F_\lambda\) [\si{\erg\per\second\per\centi\meter\cubed}]},
            ymin=0, ymax=1e+1,
        ]
        \addplot+table[hist={bins=10}, x=wl, y=f, col sep=comma] {4a.csv};
        \end{axis}
    \end{tikzpicture}
\end{center}
...
\begin{equation}
    F_\nu = F_\lambda \frac{\Delta \lambda}{\Delta \nu} = F_\lambda \frac{\Delta \lambda}{\nu_{min} + n \Delta \nu}
\end{equation}
where \(n \in [1, 10]\) is the bin number.
...
\begin{center}
    \begin{tikzpicture}
        ...
        \addplot+table[hist={bins=10}, x=wl, y=f, col sep=comma] {4b.csv};
        \end{axis}
    \end{tikzpicture}
\end{center}
