...
Since the Sun is modeled as a radiating sphere, the irradiance, or power recieved per unit area should remain constant,
\begin{equation}
    I_\odot = \SI{1.4}{\kilo\watt\per\meter\squared}
\end{equation}
The total luminosity can be measured with the equation
\begin{equation}
    I_\odot = \frac{L_\odot}{4\pi R_{\odot\oplus}^2}
\end{equation}
where \(R_{\odot\oplus}\) is the mean distance from the Earth to the Sun.
